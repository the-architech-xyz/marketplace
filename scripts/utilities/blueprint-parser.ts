...
export class BlueprintParser {
   ...
   */
  parseBlueprint(blueprintPath: string, moduleId: string, moduleType: 'adapter' | 'connector' | 'feature'): BlueprintAnalysisResult {
    try {
  ...
  private extractModuleIdFromPath(blueprintPath: string, marketplacePath: string): string {
    ...
    // Keep the directory prefix for connectors and features, remove only for adapters
    if (modulePath[0] === 'adapters') {
      modulePath.shift(); // Remove 'adapters' for adapters
    }
    // For connectors and features, keep the 'connectors/' and 'features/' prefixes
   ...
   */
  private determineModuleType(blueprintPath: string, marketplacePath: string): 'adapter' | 'connector' | 'feature' {
    const relativePath = path.relative(marketplacePath, blueprintPath);
    ...
    if (relativePath.startsWith('adapters/') || relativePath.includes('/adapters/')) {
      ...
      return 'connector'; // Default to connector for unknown types
    }
  }
}
