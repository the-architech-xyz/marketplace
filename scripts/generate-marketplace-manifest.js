  ...
  async function scanDir(dir) {
    try {
      ...
      for (const entry of entries) {
        ...
        } else if (entry.name === 'blueprint.yaml' || entry.name === 'blueprint.json' || entry.name === 'adapter.json' || entry.name === 'integration.json') {
          files.push(fullPath);
        }
      }
    } catch (error) {
...
async function processBlueprintFile(blueprintPath, category) {
  try {
    // Read blueprint file
    ...
    const moduleId = pathParts.slice(1, -1).join('/'); // Remove 'adapters/' or 'integrations/' and the config file
